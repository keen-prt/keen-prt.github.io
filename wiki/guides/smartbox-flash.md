# SmartBox Flash <OTABadge type="keenetic" text="5.0.12 (OTA)"/> <YezBadge type="keenetic" text="5.0.12" url="/assets/files/firmware/SmartBox-Flash.7z"/>
...
1. **Запуск скрипта.** Скачайте архив и запустите `Start_Flash.bat`. Скрипт сделает бэкап EEPROM, установит загрузчик Breed и перезагрузит роутер.
...
1. **Раздача файлов.** В папке с прошивкой перетащите все `bin`-файлы на `HFS.exe`.
