...
::: danger ВНИМАНИЕ
Память `FORESEE F35SQA001G` и `Airoha AN8855` не поддерживаются.
:::
...
4. После перезагрузки роутера запустите файл `CMCC_RAX3000ME_flasher.exe`, выберите вариант `Стоковая прошивка`, нажмите установить, дождитесь завершения работы скрипта, после которого роутер перезапустится и прошивка будет установлена.
::: tip Внимание
Файл пропатчит EEPROM, сгенерирует новые сервисные цифры и установит прошивку.
:::

### Установка с OpenWRT
1. Из под веб-интерфейса OpenWRT загрузить `rax3000m-me-openwrt.itb` как
   обновление.
2. После перезагрузки роутера запустите файл `CMCC_RAX3000ME_flasher.exe`, выберите вариант `OpenWrt`, нажмите установить, дождитесь завершения работы скрипта, после которого роутер перезапустится и прошивка будет установлена.
::: tip Внимание
Файл пропатчит EEPROM, сгенерирует новые сервисные цифры и установит прошивку.
:::
...
2. Из под веб-интерфейса OpenWRT загрузить `rax3000m-me-openwrt.itb` как
   обновление.
::: tip Внимание
Файл пропатчит EEPROM, сгенерирует новые сервисные цифры и установит прошивку.
:::
