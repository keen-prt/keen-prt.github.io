# Xiaomi Router 3G v1  <YezBadgeWithDropdown type="keenetic" text="4.2.3" url="/assets/files/firmware/Xiaomi-R3Gv1-4.2.3.7z" :versions="[{ text: '4.2.2', url: '/assets/files/firmware/Xiaomi-R3Gv1-4.2.2.7z' }]"/>
...
4. Перейдите в Breed по адресу 192.168.1.1. и сделайте `EEPROM` и `Full` бэкап на случай отката.
   ![альтернативный текст](/assets/images/wiki/guides/Xiaomi/breed.jpg)
5. EEPROM полученный из скрипта (`если установка была со стоковой прошивки`) или снятый из Breed (`если Keenetic ранее был установлен`) конвертируем для Keenetic через [онлайн-сервис](https://yeezyio.github.io/) выбрав `Вырезанный EEPROM` с указанием вашего MAC с этикетки. Полученный файл поместите
   в папку с прошивкой
