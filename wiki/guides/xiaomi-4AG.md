...
2. **Запуск скрипта.** Запустите `!Start.bat` и поочерёдно выберите пункты с 1 по 4. В папке `data` появятся резервная копия текущей прошивки и EEPROM.

   ![Меню установщика Breed](/assets/images/wiki/guides/Xiaomi/install-4ag.png)

3. **Подготовка EEPROM.** В [инструменте «Внести MAC»](/wiki/helpful/tools#converter) загрузите полученный `eeprom.bin` и укажите MAC-адрес с наклейки. Скачайте подготовленный EEPROM для Keenetic.
4. **Резервная копия EEPROM (если Keenetic уже был установлен).** Если ранее на устройстве уже был Keenetic, сохраните в Breed резервную копию EEPROM.

   ![Резервная копия EEPROM в Breed](/assets/images/wiki/guides/Mercusys/backup.png)
