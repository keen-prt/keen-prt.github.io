...
6. **Подготовка EEPROM.** Используйте EEPROM, созданный скриптом при установке со стоковой прошивки, либо извлеките его из `full.bin`, снятого в Breed, если Keenetic был установлен ранее. В [инструменте «Внести MAC»](/wiki/helpful/tools#converter) загрузите EEPROM, укажите MAC-адрес с наклейки и поместите созданный `eeprom.bin` в папку с прошивкой.
...
1. **Раздача файлов.** Запустите `!HFS.exe` и перетащите в его окно все `.bin`-файлы из папки с прошивкой: `U-Config`, `U-State`, `EEPROM` и `Xiaomi-R3G_xxx`.
