# Netis N6 v1 AX1800 <BoostyBadge type="keenetic" text="5.1.4"/> <OTABadge type="keenetic" text="5.1.3 (OTA)"/> <YezBadge type="keenetic" text="5.1.1" url="/assets/files/firmware/Netis-N6.7z"/>
...
1. **Установка OpenWrt.** Установите `FACTORY_N6_OpenWRT.bin` из папки `Установка Breed` поверх заводской прошивки обычным обновлением. Если OpenWrt уже был установлен — выполните сброс настроек.
...
2. **Запуск скрипта.** После запуска OpenWrt запустите `!Start.bat` **в Windows 10/11**. Скрипт сохранит резервную копию EEPROM в папку `Keenetic`, установит загрузчик и перезагрузит устройство в Breed.

   ![Работа скрипта !Start.bat](/assets/images/wiki/guides/NetisN6/script.png)

3. **Резервная копия.** Войдите в загрузчик Breed по адресу [192.168.1.1](http://192.168.1.1) и сохраните `Full dump` на случай отката прошивки.
...
1. **Раздача файлов.** Запустите `HFS.exe` и перетащите в его окно все `.bin`-файлы из папки `Keenetic`.
