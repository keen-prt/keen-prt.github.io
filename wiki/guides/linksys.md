...
5. **Резервная копия.** Перейдите в Breed по адресу `192.168.1.1` и сохраните резервную копию `Full` на случай отката к заводской прошивке.

   ![Резервная копия Full в Breed](/assets/images/wiki/guides/Xiaomi/breed.jpg)

## Установка Keenetic

1. **Раздача файлов.** Запустите `HFS`, поместив в него все файлы из архива, а также ваши калибровки (`EEPROM/factory/mtd2`). В калибровки предварительно должны быть записаны правильные MAC-адреса с помощью [EEPROM-инструментов](/wiki/helpful/tools).
