# Xiaomi Router 3P <OTABadge type="keenetic" text="5.1.4 (OTA)"/> <YezBadge type="keenetic" text="5.0.12" url="/assets/files/firmware/Xiaomi-R3P.7z"/>
...
3. **Бэкап и переход в KeenBOOT.** Скрипт сделает бэкап `factory` (он же EEPROM) в папку `installer/data` и перезагрузится в загрузчик KeenBOOT.
...
1. Загрузите `Firmware`, `EEPROM`, `U-Config` из архива по очереди через веб-интерфейс по [инструкции](/wiki/helpful/keenboot#установка-прошивки).
2. Перезагрузитесь в систему.
