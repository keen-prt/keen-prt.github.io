...
3. **Запуск скрипта.** Запустите `!Start.bat` из папки `Breed`. Скрипт установит загрузчик Breed, сохранит резервные копии заводского загрузчика и EEPROM в папку `Data`, создаст полный образ прошивки с вашим EEPROM и индивидуальными сервисными данными и поместит его в папку `Keenetic`.
4. **Резервная копия.** Зайдите в загрузчик Breed по адресу `192.168.1.1` и обязательно сохраните резервную копию `Full`. В дальнейшем её можно использовать для отката на OpenWrt — порядок действий такой же, как в следующем шаге. На этом этапе закройте `BreedEnter`.
...
- Для возврата заводского загрузчика используйте `Breed -> Upgrade -> Generic -> Bootloader`, указав файл, резервную копию которого создал скрипт.
