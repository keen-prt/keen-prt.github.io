# TP-Link EC330-G5u <YezBadge type="keenetic" text="5.1.5" url="/assets/files/firmware/TP-Link-EC330.7z"/>
...
1. **Установка OpenWrt.** [Установите OpenWrt](https://4pda.to/forum/index.php?showtopic=1030035&st=60#entry121086645). Если OpenWrt уже установлен, добавьте пакет `kmod-mtd-rw` и выполните сброс к заводским настройкам.
2. **Запуск скрипта.** Скачайте архив, распакуйте его и запустите `!Start.bat` из папки `Breed`. Скрипт сохранит резервную копию заводского загрузчика, подготовит EEPROM и U-Config, установит Breed и перезагрузит роутер.
3. **Резервная копия.** Сохраните резервную копию `Full` на случай отката прошивки.
