...
- Никак, все сборки уже идут с [максимально доступным набором](/wiki/helpful/components.md). Поскольку каждая модель имеет разную разметку, уместить все компоненты не является возможным. Просмотр установленных компонентов из под системы недоступен
...
:::
При возникновении проблем ознакомьтесь со [статьей](/wiki/helpful/mesh#решение-проблем)
...
::: info
Если установлен официальный загрузчик, и нет возможности его сменить
:::
1. Скачать файл [NAND Breed rewrite](/wiki/helpful/files)
2. Из под запущенной системы загрузить его как [обновление](/wiki/helpful/updateFirmware#способ-2). 
3. После перезагрузки зайдите в [Breed](/wiki/helpful/keenboot#вход-в-загрузчик), и выполните загрузку нужных вам файлов по инструкции со страницы модели

::: tip
В случае, если на роутере не хватает места под установку Entware, или роутер не запускается, необходимо подключиться по TFTPD64/UART и выполнить смену загрузчика.
:::
::: tip
