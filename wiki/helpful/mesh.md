...
2. Подключите роутер проводом к контроллеру
   ...
   ::: danger
   Начиная с версии 5.0.1 при переводе в режим `Ретранслятора` устройство может сохранить пароль на вход, его необходимо убрать или выполнить сброс настроек
   1. Зайти в веб-интерфейс ретранслятора
   2. Через [CLI](http://192.168.1.1/a) убрать пароль
   ```shell
   user admin no pass
   ```
   3. На контроллере выполнить команду, где `candidate` мак адрес ретранслятора, через `Tab` сам подставится нужный
   ```shell
   mws revisit {candidate} 
   ```
   :::
3. Нажмите `Захватить`. Перенос настроек может занять до 2-х минут
...
3. В разделе `Беспроводное подключение` выполните подключение к беспроводной точке контроллера
   ...
   ```
   :::
4. На контроллере выполните захват. Перенос настроек может занять до 2-х минут
   ![альтернативный текст](/assets/images/wiki/helpful/mesh/connect.png)
   ::: tip При успешном захвате роутер отобразится в списке, в названии будет его серийный номер, например `Keenetic N6 AX1800 037***462`
   :::
   ![альтернативный текст](/assets/images/wiki/helpful/mesh/mesh.png)
